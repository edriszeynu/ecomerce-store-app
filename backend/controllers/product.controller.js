    ...
    const products = await Product.aggregate([
      { $sample: { size: 3 } },
      {
        $project: {
          ...
          price: 1,
          image: 1,
          description: 1, // fixed typo
        },
      },
    ]);

    res.json(products); // return the result
  } catch (error) {
    console.log("Error getting recommended products:", error.message);
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
export const getByProductsCatagory=async(req,res)=>{
    try{
...
export const toggleFeaturedProduct=async(req,res)=>{
    try{
        ...
        res.json({message:"Product featured status toggled",product:updatedProduct});}
        else{
